...
static counter_state_t mCounter = COUNTER_0;
static uint32_t mTimerInterval = 3000; // 3 segundos iniciales
static bool mUseLongInterval = false;

osaEventId_t mMyEvents;
tmrTimerID_t myTimerID = gTmrInvalidTimerID_c;
osaTaskId_t gMyTaskHandler_ID;

#define gMyNewTaskEvent_SendPacket    (1 << 0)
#define gMyNewTaskEvent_SW3_Pressed   (1 << 1)
#define gMyNewTaskEvent_SW4_Pressed   (1 << 2)
...
static void SendCounterPacket(void) {
    ...
    // Configurar estructura del paquete
    pPacket->msgType = gMcpsDataReq_c;
    pPacket->msgData.dataReq.pMsdu = (uint8_t*)(&pPacket->msgData.dataReq + 1); // Apunta al espacio después de la estructura

    memcpy(pPacket->msgData.dataReq.pMsdu, buffer, len);

    // Configurar direcciones MAC y PAN
    ...
    pPacket->msgData.dataReq.txOptions = gMacTxOptionsAck_c; // Solicitar ACK
    pPacket->msgData.dataReq.msduHandle = mMsduHandle++;
    pPacket->msgData.dataReq.securityLevel = gMacSecurityNone_c;

    // Enviar y manejar contador de paquetes
...
void My_Task(osaTaskParam_t argument) {
    ...
    while(1) {
        ...
        if(ev & gMyNewTaskEvent_SW3_Pressed) {
            ...
            TMR_StartIntervalTimer(myTimerID, mTimerInterval, TimerCallback, NULL);
        }

        if(ev & gMyNewTaskEvent_SW4_Pressed) {
            mUseLongInterval = !mUseLongInterval;
            mTimerInterval = mUseLongInterval ? 5000 : 3000;
            TMR_StopTimer(myTimerID);
            TMR_StartIntervalTimer(myTimerID, mTimerInterval, TimerCallback, NULL);
        }
    }
