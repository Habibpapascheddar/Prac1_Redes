...
static void App_HandleMcpsInput(mcpsToNwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  case gMcpsDataInd_c:
    ...
    // Contador
    uint16_t CounterValue = *pMsgIn->msgData.dataInd.pMsdu;
	Serial_Print(interfaceId, "Valor del contador: ", gAllowToBlock_d);
	Serial_PrintHex(interfaceId, (uint8_t*)&CounterValue, 2, gPrtHexNoFormat_c);
	Serial_Print(interfaceId, "\n\r", gAllowToBlock_d);

	//Dirección de quien lo envio
    uint16_t NodeAddr = pMsgIn->msgData.dataInd.srcAddr;
    Serial_Print(interfaceId, "Mensaje recibido de: 0x", gAllowToBlock_d);
